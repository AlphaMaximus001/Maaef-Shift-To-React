...
export default function FluidSimulation() {
  ...
  useEffect(() => {
    ...
    const isWebGL2 = !!gl;
    if (!gl) {
      const canvasWebGL = canvas as any;
      gl = (canvasWebGL.getContext("webgl", params) || canvasWebGL.getContext("experimental-webgl", params)) as WebGL2RenderingContext | null;
    }
    ...
    if (isWebGL2) {
      ...
      halfFloatTexType = halfFloat ? halfFloat.HALF_FLOAT_OES : gl.FLOAT;
    }

    gl.clearColor(0.0, 0.0, 0.0, 1.0);

    const getSupportedFormat = (gl: any, internalFormat: number, format: number, type: number): any => {
    ...
    const supportRenderTextureFormat = (gl: any, internalFormat: number, format: number, type: number): boolean => {
    ...
    const resizeFBO = (target: any, w: number, h: number, internalFormat: number, format: number, type: number, param: number) => {
    ...
    const resizeDoubleFBO = (target: any, w: number, h: number, internalFormat: number, format: number, type: number, param: number) => {
      if (target.width === w && target.height === h) return target;
      
      // Delete old textures/fbos to prevent leakage
      ...
      gl!.deleteFramebuffer(oldRead.fbo);
      gl!.deleteTexture(oldRead.texture);
      gl!.deleteFramebuffer(oldWrite.fbo);
      gl!.deleteTexture(oldWrite.texture);
    ...
    const getTextureScale = (texture: any, width: number, height: number) => {
      return {
    ...
    // Framebuffer / Simulation Objects
    let dye: any;
    let velocity: any;
    let divergence: any;
    let curl: any;
    ...
    const renderDisplay = (target: any) => {
      ...
      if (!config.TRANSPARENT) {
        ...
        colorProgram.bind();
        gl!.uniform4f(colorProgram.uniforms.color, c.r / 255, c.g / 255, c.b / 255, 1);
        blit(target);
      }
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      pointer.down = true; // FORCE DRAG ACTION ON HOVER
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        let canvasRect = canvas.getBoundingClientRect();
        let posX = scaleByPixelRatio(touches[i].clientX - canvasRect.left);
        let posY = scaleByPixelRatio(touches[i].clientY - canvasRect.top);
    ...
    // ─── DISPOSAL AND CLEANUP (PREVENTS MEMORY LEAK) ───
    return () => {
      ...
      // Delete WebGL buffers, textures, compiled shaders, and framebuffers
      ...
      // Delete compiled shader programs
      const programs = [
